...
- **Status Checks** checks physical machine (underlying hyper-visor) and returns pass & fail.
  ...
  - Auto Scaling Group uses this check by default. Can set to use Eastic Load Balancer's health check.
...
- **Enhanced Networking** can be enabled on specific instances with a driver to reduce networking overhead + faster networking.
  ...
  - Elastic Fabric Adapter - High Perfomance Computing - Linux machines only
- **Monitoring**
...
- **Spot instances**
  - Cheapest (up to 90% discount compared to on demand) but risk of losing instance
    ...
    - If aws shut down your instance in first hour you don't pay, but you have to pay if you terminate.
