...
- It supports [Gateway VPC endpoints](./04-02-networking-vpc.md#vpc-endpoints) for VPC access without Internet.
...
- 📝 Allows granting access (URL) to one or more users for a certain amount of time and expire it.
  - URL can be used both for download (`GET`) or upload (`PUT`).
...
- ❗ It does not work with CloudFront
  - 💡 Use instead CloudFront signed URLs
...
- **Logging and Audit**
  ...
  - **Access logs vs Object-Level CloudTrail logs**
    ...
    | **Delivery** | Timeliness & delivery are not guaranteed | API calls within 15 min are captured and sent within 5 min |
  ...
  - Data can be analyzed using data analysis tools
    - Also [**Amazon Athena**](./06-01-03-data-s3-integrated-services-snowball-athena-storage-gateway-object-lambda-transfer.md#amazon-athena) and **S3 Select** can be used to query access logs
  ...
  - 📝 **Bucket policies**
    ...
    - **Syntax for including root and its endpoints**
      - ❗ Careful: In policies if you use `arn` *(amazon resource name)* with only `/` e.g.it'll not apply policy to all endpoints under it you need to define it as with `/*`.
        - E.g. `arn:aws:s3:::thebucketname/*` instead of `arn:aws:s3:::thebucketname/`.
        - ARN with `/*` will not include the root URL, if you need root as well use both `/` and `/*`.
...
- **Encryption at rest**
  ...
  - 4 methods of encrypting objects:
    ...
    2. **SSE-KMS** (Server Side Encryption - Key Management Service)
       - Leverage [AWS Key Management Service](./02-03-security-encryption-ssm-kms-secrets-manager-certificate-manager.md#aws-key-management-service) to manage encryption
       - KMS advantages: user control (over rotation of key) + audit trail (how the key was used)
       - Object is encrypted server side
       - 📝 Must set header `"x-amz-server-side-encryption":"aws:kms"`
       - AWS KMS generates & uses AWS KMS key as encryption key
         - Formerly called Customer Master Keys (CMKs) - rebranded in 2021
    ...
    3. **SSE-C** (Server Side Encryption - Customer)
       ...
       - Uses client-provided data key
       - Amazon does the encryption server-side but you manage the keys
  ...
  - Responsibilities
    ...
    |---:|:------------------------:|:---------------------:|:----------------------:|
    | **SSE-S3** | AWS | AWS | AWS |
    | **SSE-KMS** (AWS managed KMS key) | AWS | AWS | AWS |
    | **SSE-KMS** (customer managed KMS key) | AWS  | AWS | you |
    | **SSE-C** | AWS | you | you |
    | **AWS SDK + KMS** (AWS managed KMS key) | you | AWS | AWS |
    | **AWS SDK + KMS** (customer managed KMS key) | you | AWS | you |
...
- **Encryption in transit (SSL)**
  - 📝Achieved by SSL/TLS
  - S3 exposes HTTP and HTTPS URL, HTTPS has encryption in flight.
- **Default Encryption**
...
- 📝 Helps discover sensitive data across your S3
  - E.g. PII (Personal identifiable information) such as credit card number

- Scans both S3 scanning buckets + CloudWatch logs
- Publishes findings as events to [Amazon EventBridge](./08-01-02-event-broadcasting-sns-eventbridge.md#amazon-eventbridge)
  - [Amazon EventBridge](./08-01-02-event-broadcasting-sns-eventbridge.md#amazon-eventbridge) can route it to Lambda, S3, SNS Amazon Kinesis etc.

## S3 Access Points

- Named network endpoints
- Each access point gets its own hostname and [access policy](#s3-policies)
- Benefits:
  - Replace complex bucket policies
  - Unique names and permissions
  - You can restrict access points to only work within your VPC
- Cost: No additional charges - you just pay normal S3 request rates.
